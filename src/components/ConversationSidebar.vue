...
<template>
  <aside class="sidebar" :class="{ collapsed: props.collapsed }">
    <header class="sidebar-header">
      ...
      <n-button class="sidebar-collapse-button" circle quaternary :title="copy.collapseSidebar" :aria-label="copy.collapseSidebar" @click="$emit('toggle-collapse')"><template #icon><n-icon><ChevronLeftIcon /></n-icon></template></n-button>
    </header>
    ...
    <nav class="conversation-list" :aria-label="copy.recentConversations">
      ...
      <div v-for="conversation in props.conversations" :key="conversation.id" class="conversation-item" :class="{ active: conversation.id === props.activeId }">
        ...
        <n-button class="conversation-delete" quaternary circle :disabled="props.disabled" :title="copy.deleteConversation" :aria-label="`${copy.deleteConversation}: ${conversation.title}`" @click.stop="$emit('delete', conversation.id)"><template #icon><n-icon><TrashIcon /></n-icon></template></n-button>
      </div>
    </nav>
    <n-button class="sidebar-settings-button" quaternary :disabled="props.disabled" @click="$emit('settings')"><template #icon><n-icon><SettingsIcon /></n-icon></template>{{ copy.settings }}</n-button>
    <div class="sidebar-resize-handle" role="separator" aria-orientation="vertical" @mousedown="$emit('resize-start', $event)"></div>
  </aside>
</template>
...
:global(.is-resizing-sidebar) .sidebar-resize-handle {
  background: rgb(31 111 235 / 25%);
}
...
.sidebar-new-button:hover:not(:disabled) {
...
.conversation-item.active .conversation-select:hover:not(:disabled) {
  background: transparent;
}
